...
function cmdCurrent(flags: Flags) { const index = loadIndex(); const active = index.activeVaultProfile; printRefreshHint(flags, 'openclaw-team-vault current -r'); if (active && index.profiles[active]) { const meta = index.profiles[active]; const data = flags.refresh ? refreshUsageFor(active) : cachedUsageFor(active); const usage = data.usage || { fiveHourLeft: null, fiveHourReset: null, weekLeft: null, weekReset: null }; console.log(`${bullet(true)} ${c(active, 'bold', 'cyan')} ${c(`(${meta.label || active})`, 'dim')}`); console.log(`  ${c('account', 'dim')}: ${shortId(meta.accountId)}`); printExpiry(meta); printRefreshInfo(data.refreshedAt, flags.refresh); console.log(`  ${formatUsageLine('5h', usage.fiveHourLeft, usage.fiveHourReset)}`); console.log(`  ${formatUsageLine('week', usage.weekLeft, usage.weekReset)}`); } else console.log(c('(active team not mapped in vault)', 'yellow')); }
function cmdLs(flags: Flags) { const index = loadIndex(); const names = Object.keys(index.profiles || {}); if (!names.length) return console.log('No teams saved. Use: openclaw-team-vault add <name> [--days N]'); if (!flags.less) printRefreshHint(flags, 'openclaw-team-vault ls -r'); const results: Row[] = names.map((name) => ({ name, data: flags.refresh ? refreshUsageFor(name) : cachedUsageFor(name), isActive: index.activeVaultProfile === name, meta: loadIndex().profiles[name] || {} })); const sorted = sortRows(results, flags.order); if (!flags.less) sorted.forEach((row, i) => printTeamUsage(row.name, row.data.usage, row.isActive, row.data.refreshedAt, flags.refresh, i > 0)); const total5h = sumDefined(results.map((r) => r.data.usage?.fiveHourLeft)); const totalWeek = sumDefined(results.map((r) => r.data.usage?.weekLeft)); if (flags.less) { console.log(`usage source: ${flags.refresh ? 'refreshed' : 'cached'} · order: ${flags.order}`); for (const row of sorted) { const u = row.data.usage || {}; console.log(`${row.isActive ? '*' : '-'} ${row.name}  5h:${u.fiveHourLeft ?? '?'}%  week:${u.weekLeft ?? '?'}%`); } console.log(`total 5h:${total5h ?? '?'}%  week:${totalWeek ?? '?'}%`); return console.log('usage: openclaw-team-vault use <name>'); } console.log(c('════════════════════════════════════════', 'gray')); console.log(`${c('Σ total', 'bold', 'cyan')} ${c('cached usage sum', 'dim')}`); console.log(`  ${c('5h', 'cyan')}    ${total5h == null ? '?' : c(`${total5h}%`, 'bold')}`); console.log(`  ${c('week', 'cyan')}  ${totalWeek == null ? '?' : c(`${totalWeek}%`, 'bold')}`); }
function cmdShow(name: string | undefined, flags: Flags) { if (!name) fail('Usage: openclaw-team-vault show <name> [--refresh]'); const index = loadIndex(); if (!index.profiles[name]) fail(`Unknown team: ${name}`); printRefreshHint(flags, `openclaw-team-vault show ${name} -r`); const data = flags.refresh ? refreshUsageFor(name) : cachedUsageFor(name); printTeamUsage(name, data.usage, index.activeVaultProfile === name, data.refreshedAt, flags.refresh); }
